...
(**let simpler_sum xs = List.fold_left (+) 0 xs;;**)
let simpler_product xs = List.fold_left (fun a x -> a * x) 1 xs;;
...
let float_of_int xs = List.map (Float.of_int) xs
(** ========== [List.init] ========== **)
...
let range from to_ = List.init (to_-from) (fun x -> x+from)
